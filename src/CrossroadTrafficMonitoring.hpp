...
inline const char* ToString(VehicleCategory cat)
{
    switch (cat)
    {
    case VehicleCategory::Bicycle: return "Bicycle";
    case VehicleCategory::Car:     return "Car";
    case VehicleCategory::Scooter: return "Scooter";
    }
    return "Unknown";
}
...
template <typename T>
void CrossroadTrafficMonitoring_OnSignal_Helper(class CrossroadTrafficMonitoring* self, const T& vehicle);
...
private:
    ...
    template <typename T>
    friend void CrossroadTrafficMonitoring_OnSignal_Helper(CrossroadTrafficMonitoring*, const T&);
    ...
    using CategoryMemberOption = boost::intrusive::member_hook<
        Vehicle, Vehicle::Hook, &Vehicle::category_hook>;

    // **Disable constant_time_size** because auto_unlink is being used.
    using CategoryList = boost::intrusive::list<
        Vehicle, 
        CategoryMemberOption,
    ...
    using AlphaMemberOption = boost::intrusive::member_hook<
        Vehicle, Vehicle::Hook, &Vehicle::alphabetical_hook>;

    using AlphabeticalList = boost::intrusive::list<
        Vehicle,
        AlphaMemberOption,
        boost::intrusive::constant_time_size<false>
    >;
